...
class myMaps(arcade.Window):
    def __init__(self):
        ...
        self.player = None
        self.coin = None
        self.viewport = None  # Corrected initialization
    ...
    def setup(self):
        ...
        self.viewport = arcade.View(width=500, height=500)
        self.viewport.set_window(self)

    def on_draw(self):
        arcade.start_render()

        # Use the viewport to draw
        self.viewport.use()
        self.bg.draw()
        self.item.draw()
        self.ground.draw()
    ...
    def on_update(self, delta_time: float):
        ...
        if self.player.left < 0:
            self.player.left = 0
        elif self.player.right > self.Map1.width * tileSize:
            self.player.right = self.Map1.width * tileSize

        # Update the viewport's position to follow the player
        self.viewport.set_position(self.player.center_x, self.player.center_y)
